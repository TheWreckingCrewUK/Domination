...
*/
sideMissionLocationArray = nearestLocations [_pos, ["NameVillage","NameCity","NameCityCapital"], 3000];

if(str sideMissionLocationArray == "[]") exitWith{
	isSideMissionComplete = 1;
};
_rand = round (random(count sideMissionLocationArray));
while {_rand == 0} do {_rand = round (random(count sideMissionLocationArray));};
...
//side Mission hint
parseText format["<t align='center'><t size='2' color='#008080'>Mortars At </t><br/><t align='center'><t size='1.5' color='#ffffff'> %1</t><br/><t align='center'>---------------------<br/></t><t align='left'><t size='1' shadow='1.1' shadowColor='#000000' color='#d8c08d'>The enemy has mortars nearby that will assist in defending the AO. Destroy them.</t>", _name] remoteExec ["hint"];

//creates the task
[West,[_name],["Hostiles are reinforcing their objective with Mortars. Destroy it before it fires on the AO",format["Destroy Mortars in %1",_name],""],objNull,True,1,True,"",False] call BIS_fnc_taskCreate;
...
_markerstr setMarkerType "o_mortar";

//trigger that checks if mortars are alive.
if isServer then {
...
//spawns 4 mortars and has them face the AO. Also spawns 2 defend sections.
for "_i" from 1 to 1 do {
	if isServer then {
		_pos = [_pos,[200,300],random 360,0,[1,100]] call SHK_pos;
		{
			_spawnPos = [_pos,[0,10],random 360,0,[0]] call SHK_pos;
			_unit = [_spawnPos, EAST, mortar] call BIS_fnc_spawnGroup;
			_unit = leader _unit;
		...
		for "_i" from 1 to 2 do {
			...
			[_unit, _pos, 150] call TWC_Domination_fnc_patrol;
		};
...
if(bunkerCount != 3 || towerCount != 1 || areaCleared != 1) then{
	...
	parseText format["<t align='center'><t size='2' color='#008080'>Side Mission</t><br/><t align='center'><t size='1.5' color='#00FF00'> Completed</t><br/><t align='center'>---------------------<br/></t><t align='left'><t size='1' shadow='1.1' shadowColor='#000000' color='#d8c08d'>The enemy mortars in %1 have been destroyed. Good Job!</t>", _name] remoteExec ["hint"];
}else{
