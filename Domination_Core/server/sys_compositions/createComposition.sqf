...
_objs = [];
_objs = call (compile (preprocessFileLineNumbers format ["Domination_Core\server\sys_compositions\%1.sqf",_script]));
private ["_posX", "_posY"];
_posX = _pos select 0;
_posY = _pos select 1;
_newObjs = [];
...
compositionObjects = compositionObjects + _newObjs;