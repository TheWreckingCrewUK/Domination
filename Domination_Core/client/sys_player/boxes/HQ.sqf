if (["HQ", str (group player)] call BIS_fnc_inString) then {
	...
	[_x,0,["ACE_MainActions", "ammospawn", "ammospawnheavy"],_actionempty,true] call ace_interact_menu_fnc_addActionToClass;
	...
	if (!(isnil "twc_missionname")) then {
		if ((["90", twc_missionname] call BIS_fnc_inString)) then {
			...
			[_x,0,["ACE_MainActions", "ammospawn", "ammospawnlight"],_90UKaction,true] call ace_interact_menu_fnc_addActionToClass;
			} else {
