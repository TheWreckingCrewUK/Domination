if (["HQ", str (group player)] call BIS_fnc_inString) then {
	...
	if (!(isnil "twc_missionname")) then {
		if ((["90", twc_missionname] call BIS_fnc_inString)) then {
			_90UKaction = ["Spawnsmall90UKCreate","Spawn Small UK Crate","",{execvm "insurgency_core\client\sys_player\boxes\supply_boxes\smallcrateuk90.sqf"},_condition] call ace_interact_menu_fnc_createAction;
			...
			if ((["00", twc_missionname] call BIS_fnc_inString)) then {
				_hUKaction3 = ["SpawnsmallUKCreate","Spawn Small UK Crate","",{execvm "insurgency_core\client\sys_player\boxes\supply_boxes\smallcrateuk2000.sqf"},_condition] call ace_interact_menu_fnc_createAction;
