if((typeOf player) in ["Modern_pol_commander","Modern_pol_commander_d", "Modern_pol_subcommander","Modern_pol_subcommander_d"])then{
	...
	_alphaaction = ["SpawnsmallAlphaCreate","Spawn TOW Launcher","",{execvm "domination_core\client\sys_player\boxes\supply_boxes\smallcratetow.sqf"},_condition] call ace_interact_menu_fnc_createAction;
