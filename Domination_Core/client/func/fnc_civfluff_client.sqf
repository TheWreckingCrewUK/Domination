...
_isnearbase = ((((getpos _player) distance twc_basepos ) < 300) || (((getpos _player) distance (getmarkerpos "respawn_west_forwardbase") ) < 300));
if (!_isnearbase) exitwith {
	[twc_fnc_civfluff_client, [_player], (_timer * 10)] call CBA_fnc_waitAndExecute;
	//systemchat "civfluffclient 9";
};


_buildings = nearestObjects [(_player getRelPos [(((speed _player) + 0.1) * 6), 0]), ["house", "vysilacka"], 200];
//this is set to 0 because domi/annex bases commonly have no recognised buildings
