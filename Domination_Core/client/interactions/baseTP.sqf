_alphaaction = ["SpawnsmallAlphaCreate","Teleport to Patrol Base","",{player setpos (getmarkerpos "respawn_west_forwardbase");[player] execvm "domination_core\client\sys_player\vehicledrop.sqf"},{((str (getmarkerpos "respawn_west_forwardbase")) != "[0,0,0]") && (!(player getvariable ["ace_dragging_isdragging", false])) && (!(player getvariable ["ace_dragging_iscarrying", false]))}] call ace_interact_menu_fnc_createAction;
